...
const Mealplan = require("../model/Mealplan");
const MealPlanData = require("../model/mealData");
...
const createMealPlan = async (req, res, next) => {
  try {
    ...
    if (mealPlanData.days && mealPlanData.days.length > 0) {
      ...
      if (
        mealPlanData.creationMethod ===
        "Merge selected days into a single version"
      ) {
        const existingDays = await MealPlan.find({
          clientId,
          days: { $in: mealPlanData.days },
        });
        for (const existingRecord of existingDays) {
          const allDaysMatch = existingRecord.days.every((day) =>
            mealPlanData.days.includes(day)
          );
          if (allDaysMatch) {
            const del_obj = await MealPlan.deleteOne({
        ...
        const everyDayPlan = await MealPlan.findOne({
          clientId,
          days: { $in: mealPlanData.copyMealPlan },
        });
        ...
        res.status(201).json({
          data: mealPlanData,
          message: "Meal plan created or updated successfully",
        });
      } else if (
        mealPlanData.creationMethod === "Create a version for each day"
      ) {
        ...
        for (const day of selectedDays) {
          const existingPlanForDay = await MealPlan.findOne({
            clientId,
            days: [day],
          });
...
const updateMealPlan = async (req, res, next) => {
  try {
    ...
    const pop = await MealPlan.findOne({ _id: mealId });
    ...
    return res
      .status(200)
      .json({ message: "Meal plan updated successfully", pop: modifiedPop });
  } catch (error) {
    console.log(error);
    next(error);
  }
...
const deleteMealPlan = async (req, res, next) => {
  try {
    ...
    const everyDayRecord = await MealPlan.findOne({
      clientId,
      days: { $in: "Every Day" },
    });
    if (everyDayRecord) {
...
const deleteParticularFood = async (req, res, next) => {
  try {
    ...
    const meal = mealPlan.mealPlans
      .map((meal) =>
        mealTypes.map((type) =>
          meal[type].filter((item) => item._id.toString() === objectId)
        )
      )
      .flat()
    ...
    if (submeal) {
      for (const submealArray of submeal) {
        for (const subfoodArray of submealArray) {
          const indexToDelete = subfoodArray.subfoods.findIndex(
            (subfood) => subfood.foodId.toString() === Id
          );
          if (indexToDelete !== -1) {
...
const createMealData = async (req, res, next) => {
  try {
    const data = await MealPlanData.create({
      ...req.body,
    });

    return res
      .status(200)
      .json({ message: "meal data created successfully", data: data });
  } catch (error) {
    return res.status(401).json({ message: error.message });
  }
};

const getMealData = async (req, res, next) => {
  try {
    const data = await MealPlanData.find();
    if (data.length === 0) {
      return res
        .status(400)
